...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(name).info(f"Gettings Assistants Info...")
        if config.STRING1:
        ...
        if config.STRING2:
            ...
            try:
                await self.two.send_message(
                    config.LOG_GROUP_ID, f"*⩹━★⊷⌯𝐒𝐎𝐔𝐑𝐂𝐄 𝐇𝐀𝐘𝐀 ⌯⊶★━⩺*\n*» {config.MUSIC_BOT_NAME} ᴀssɪsᴛᴀɴᴛ ᴛᴡᴏ sᴛᴀʀᴛᴇᴅ :**\n\n✨ ɪᴅ : {self.two.id}\n❄ ɴᴀᴍᴇ : {self.two.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.two.username}"
                )
            except:
                LOGGER(name).error(
        ...
        if config.STRING4:
            ...
            try:
                await self.four.send_message(
                    config.LOG_GROUP_ID, f"*⩹━★⊷⌯𝐒𝐎𝐔𝐑𝐂𝐄 𝐇𝐀𝐘𝐀 ⌯⊶★━⩺*\n*» {config.MUSIC_BOT_NAME} ᴀssɪsᴛᴀɴᴛ ғᴏᴜʀ sᴛᴀʀᴛᴇᴅ :**\n\n✨ ɪᴅ : {self.four.id}\n❄ ɴᴀᴍᴇ : {self.four.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.four.username}"
                )
            except:
                LOGGER(name).error(
        ...
        if config.STRING5:
            ...
            try:
                await self.five.send_message(
                    config.LOG_GROUP_ID, f"*⩹━★⊷⌯𝐒𝐎𝐔𝐑𝐂𝐄 𝐇𝐀𝐘𝐀 ⌯⊶★━⩺*\n» {config.MUSIC_BOT_NAME} ᴀssɪsᴛᴀɴᴛ ғɪᴠᴇ sᴛᴀʀᴛᴇᴅ :**\n\n✨ ɪᴅ : {self.five.id}\n❄ ɴᴀᴍᴇ : {self.five.name}\n💫 ᴜsᴇʀɴᴀᴍᴇ : @{self.five.username}"


)
            except:
                LOGGER(name).error(
                    f"Assistant Account 5 has failed to access the log Group. Make sure that you have added your assistant to your log group and promoted as admin ! Source HaYa"
                )
                sys.exit()
            LOGGER(name).info(
